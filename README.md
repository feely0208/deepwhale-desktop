...
- **背景皮肤**：选一张你喜欢的图片，**完全覆盖原界面**——图片复制到 `userData/skins/`，全窗口 cover 平铺（data URI 内联，换图即时生效），界面层自动半透明让图透出，并**拓展到左侧工作区栏**（该侧淡化 + 渐变过渡），背景可见度可调（30%~100%）。
- 入口：托盘菜单 → 皮肤，或 **设置页 → 皮肤** 栏（在 通用设置/模型/插件/Agent 预设 四项之下顺延）。
- 自定义 CSS（`userData/custom.css`）保存即时重注入，可做精细微调。

### 桌面宠物
- 默认宠物：Codex 风格橙色泪滴（依据基图《Codex 图像 2026年8月3日 23_02_34》像素分析重绘，SVG 渐变 + CSS 动画：呼吸弹跳/眨眼/挥手/打盹）。
- 切换入口：右键宠物、托盘菜单 → 宠物，或 **设置页 → 宠物** 栏（预览/开关/穿透点击/宠物工坊）。
- **自制宠物**：托盘菜单 → 宠物 → **宠物工坊…**（SVG 实时编辑预览保存、导入图片自动去白底生成透明宠物），或把 `.gif` / `.svg` / `.png` / `.jpg` / `.webp` 丢进宠物目录（"打开宠物目录…"）即可切换。
- **帧动画宠物**（Codex 标准）：宠物目录下放 `名字/` 文件夹（内含 `manifest.json` + `spritesheet.png`，192×208 格子、行=状态），桌面宠物按帧动画播放：空闲呼吸、悬停挥手、点击跳跃、拖拽跑动、周期"办公桌工作"等；内置示例：`红领`（红领带职业包裙，含"电脑前工作"动作帧）。
